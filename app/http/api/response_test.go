...
func TestOKResponse(t *testing.T) {
	...
	t.Run("error on encoding response", func(t *testing.T) {
		...
		// Simulate an error by passing a channel which cannot be marshalled to JSON
...
func TestWriteErrorJSON(t *testing.T) {
	t.Run("write error JSON with valid input", func(t *testing.T) {
		recorder := httptest.NewRecorder()
		writeErrorJSON(recorder, http.StatusBadRequest, "Invalid request")
