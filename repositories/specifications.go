...
func NewSpecificationsRepository(db *gorm.DB) *SpecificationsRepository {
...
func (r *SpecificationsRepository) FindByProductID(productID uint) ([]models.Specification, error) {
	var specs []models.Specification
	err := r.db.Where("product_id = ?", productID).Find(&specs).Error
	return specs, err
}
...
func (r *SpecificationsRepository) Delete(id uint) error {
	return r.db.Delete(&models.Specification{}, id).Error
}
