...
func TestHandleCreate(t *testing.T) {
	...
	t.Run("bad json", func(t *testing.T) {
		...
		recorder := httptest.NewRecorder()
		req, _ := http.NewRequest("POST", "/brands", bytes.NewReader([]byte("invalid json")))
	...
	t.Run("db error", func(t *testing.T) {
		...
		reqBody := inbound.CreateBrandRequest{Name: "Brand1"}
		bodyBytes, _ := json.Marshal(reqBody)
...
func TestHandleUpdate(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		reqBody := inbound.UpdateBrandRequest{Name: "UpdatedBrand"}
		bodyBytes, _ := json.Marshal(reqBody)

		brand := reqBody.ToDomain()
		brand.ID = 1
		repo.EXPECT().Update(brand).Return(nil)

		recorder := httptest.NewRecorder()
		req, _ := http.NewRequest("PUT", "/brands/1", bytes.NewReader(bodyBytes))
		req.SetPathValue("id", "1")
	...
	t.Run("invalid id", func(t *testing.T) {
		...
		recorder := httptest.NewRecorder()
		req, _ := http.NewRequest("PUT", "/brands/invalid", bytes.NewReader([]byte("{}")))
		req.SetPathValue("id", "invalid")
	...
	t.Run("bad json", func(t *testing.T) {
		...
		recorder := httptest.NewRecorder()
		req, _ := http.NewRequest("PUT", "/brands/1", bytes.NewReader([]byte("invalid json")))
		req.SetPathValue("id", "1")
	...
	t.Run("db error", func(t *testing.T) {
		...
		brand := reqBody.ToDomain()
		brand.ID = 1
		repo.EXPECT().Update(brand).Return(errors.New("db error"))
...
func TestHandleDelete(t *testing.T) {
	...
	t.Run("invalid id", func(t *testing.T) {
		...
		recorder := httptest.NewRecorder()
		req, _ := http.NewRequest("DELETE", "/brands/invalid", nil)
		req.SetPathValue("id", "invalid")
